                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('products.update','test') }}" method="POST" enctype="multipart/form-data">                            
                                    ...
                                    <select class="form-select" name="category_id" id="" aria-label="Default select example">
                                        ...
                                        @foreach ($cate_levels as $category)
                                            ...
                                            @if (count($category->childs) > 0)
                                                @include('backend.products.editsubcategories', ['subcategories' => $category->childs, 'parent' => $category->name,'product'=> $product])
                                            @endif                                                    
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col">
                                    <div class="form-floating">
                                        <textarea class="form-control" name="description"
                                            placeholder="Leave a comment here" id="floatingTextarea">{{ $product->description }}</textarea>
                                        <label for="floatingTextarea">{{ trans('products_trans.description') }}</label>
                                    </div>
                                </div>
                            </div>
                            <!-- product image-->
                            <div class="text-muted small fx-bold text-uppercase px-3">{{ trans('products_trans.image') }}
                            </div>
                            <hr class="dropdown-divider mb-2" />
                            <div class="row mb-2">
                                <div class="col">
                            ...
                            <div class="row mb-2">
                                <div class="col">
                                    <div class="mb-3">
                                        <img src="{{ $product->image_path }}" style="width: 300px;" class="img-thumbnail image-preview" alt="">
                                    </div>
                                </div>
                            </div>
